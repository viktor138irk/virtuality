...
- Работаем прямо в `main` (так решил владелец). Перед каждым пушем — `make check`; не пушить
  недоделанное: ноды на канале «Ранний доступ» (`main`) подтягивают main каждую ночь.
- Узлы на канале `stable` (по умолчанию) обновляются только до тегов `vX.Y.Z`. Релиз: поднять `VERSION`,
  добавить запись в `updates/versions.json`, запушить, поставить тег `vX.Y.Z` на main — тег же запускает
  сборку ISO в GitHub Actions (artifacts `virtuality-iso-amd64/arm64`).
